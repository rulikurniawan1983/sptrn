...
class PerikananRepository
{
    ...
    public function __construct(Perikanan $model, KecamatanRepository $kecamatanRepository, DesaRepository $desaRepository, TingkatResikoRepository $tingkatResikoRepository, SkalaUsahaRepository $skalaUsahaRepository, KbliRepository $kbliRepository, PengesahanBadanHukumRepository $pengesahanBadanHukumRepository, JenisUsahaRepository $jenisUsahaRepository, JenisPerikananRepository $jenisPerikananRepository, StatusPermodalanRepository $statusPermodalanRepository, StatusVerifikasiRepository $statusVerifikasiRepository)
    {
        ...
        $this->jenisPerikananRepository      = $jenisPerikananRepository;
    ...
    public function customCreateEdit($data, $item = null)
    {
        ...
        if ($section == "form") {
            $data += [
                ...
                'listJenisPerikanan' => $this->jenisPerikananRepository->getAll()->pluck('nama', 'id')->toArray(),
            ];
    ...
    {
        if ($method == "store") {
            return redirect()->route('perikanan.edit', $model->id)->with('success', trans('message.success_save'));
        } else {
