...
class PeternakanImport implements ToCollection, WithHeadingRow, WithCalculatedFormulas
{
    /**
    ...
    */
    ...
    public function collection(Collection $rows)
    {
        ...
        {
            $nama = $row['nama'];
            $nib = str_replace("'", "", $row['nib_nomor']);
            // $check = Peternakan::where("nama", $nama)->where("nib", $nib)->first();
            $check = null;
            if ($check == null) {
                $data = [
                    "nama" => $nama,
                    "id_kbli" => str_replace("'", "", $row['kode_kbli']),
                    "id_jenis_usaha" => $row['kode_jenis_badan_usaha'],
                    "id_jenis_peternakan" => $row['kode_jenis_peternakan'],
                    "id_kecamatan" => $row['kode_kecamatan'],
                    "id_kelurahan" => $row['kode_kelurahan'],
