...
<head>
    ...
    <title>{{ $titlePage }} - Spartan</title>
    <link rel="shortcut icon" href="{{ asset('assets/img/17350108258803.png') }}">
    
    <!-- Required Bootstrap CSS from Sneat (only core for layout utilities) -->
...
<body>
    <div class="container">
        <div class="card">
            <h1 class="card_title">Selamat Datang! 👋</h1>
            <p class="card_title-info">Silakan masuk ke akun Anda.</p>
            ...
            @endif
            @if (session('success_register_and_verify'))
            @include('auth.resend-verification')
            @endif
            @if (session('success'))
            ...
            {{ html()->form('POST', route('login.action'))->class('card_form')->attribute('enctype', 'multipart/form-data')->id('form')->open() }}
                <div class="input">
                    <input type="email" name="email" id="email" class="input_field" required autofocus tabindex="1" value="{{ old('email') }}" />
                    <label class="input_label">Alamat email</label>
                </div>
                <div class="input">
                    <input type="password" name="password" id="password" class="input_field" required tabindex="2" />
                    <label class="input_label">Nomor NIK</label>
                    <span class="input_eye" id="password-toggle" style="cursor: pointer; z-index: 10;">
                        <!-- Eye icon SVG -->
                        <svg viewBox="0 0 146 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M143 37C143 45.4902 136.139 53.9606 123.263 60.487C110.554 66.9283 92.7879 71 73 71C53.2121 71 35.446 66.9283 22.7375 60.487C9.86096 53.9606 3 45.4902 3 37C3 28.5098 9.86096 20.0394 22.7375 13.513C35.446 7.07167 53.2121 3 73 3C92.7879 3 110.554 7.07167 123.263 13.513C136.139 20.0394 143 28.5098 143 37Z" stroke-width="6" stroke="#8597a3"/>
                            <circle cx="73" cy="37" r="34" stroke-width="6" stroke="#8597a3"/>
                        </svg>
                    </span>
                </div>
                
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 15px; position: relative; z-index: 10;">
                    <div class="form-check" style="font-size: 14px;">
                        <input type="checkbox" name="remember" id="remember-me" class="form-check-input" tabindex="3">
                        <label for="remember-me" class="form-check-label" style="cursor: pointer;">Ingatkan Saya</label>
                    </div>
                    <a href="{{ route('password.request') }}" tabindex="6" style="font-size: 14px; text-decoration: underline; z-index: 10; position: relative;">Lupa Password?</a>
                </div>

                <button type="submit" class="card_button" tabindex="4">Sign in</button>
                <a href="{{ route('front.home.index') }}" class="card_button" style="background: transparent; color: #334155; border: 1px solid #cbd5e1; margin-top: 10px; text-decoration: none; display: block; text-align: center;">Kembali ke Beranda</a>
            {{ html()->form()->close() }}
            
            <div class="card_info" style="text-align: center; margin-top: 20px; z-index: 10; position: relative;">
                <p>Belum punya akun? <a href="{{ route('register', ['jenis' => 'UMKM']) }}" style="text-decoration: underline;">Daftar di sini</a></p>
            </div>
        </div>
