...
@section('content')
    <x-tab-praktek-dokter-hewan-component :item="$getParent" :active-tab="$titlePage" :is-lock="false" />
            ...
            <div class="mt-2 mt-md-0 d-flex flex-column flex-sm-row gap-2">
                @if (!@$disabled_add)
                    @can("$permission_main Add")
                        <a href="{{ @$routeAddCustom ? @$routeAddCustom : route("$route.create", ['id_praktek_dokter_hewan' => $getParent->id]) }}" class="btn btn-primary">
                            <i class="ti ti-plus"></i> {{ __('message.add') }}
                        </a>
                        <a href="{{ route("$route.create-multiple", ["id_praktek_dokter_hewan" => $getParent->id]) }}" class="btn btn-info">
