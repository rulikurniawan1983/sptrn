            ...
            <div class="card shadow-sm">
                <div class="card-header border-bottom py-3 d-md-flex justify-content-between align-items-center">
                    ...
                    <div class="mt-2 mt-md-0">
                        <a href="{{ @$backroute ? @$backroute : route("$route.index", ['id_praktek_dokter_hewan' => $getParent->id]) }}" class="btn btn-secondary">
...
<script>
    ...
    doc_upload.on("sending", function(a, b, c) {
        ...
        c.append("id_praktek_dokter_hewan", $('[name=id_praktek_dokter_hewan]').val());
        c.append("id_jenis_galeri", $('[name=id_jenis_galeri]').val());
        c.append("nama", $('[name=nama]').val());
    });

    doc_upload.on("queuecomplete", function(file) {
        window.location.href = "{{ url($route) }}?id_praktek_dokter_hewan=" + $("[name=id_praktek_dokter_hewan]").val();
    });
